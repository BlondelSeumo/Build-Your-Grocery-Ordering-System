...
class LicenseBoxAPI
{
	...
	{
		$this->product_id = '4B7E9975';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '87D7B0BCD7587-orange';
	...
	private function call_api($method, $url, $data)
	{
		...
		$this_server_name = getenv('SERVER_NAME') ?:
										...
										getenv('HTTP_HOST') ?:
										$_SERVER['HTTP_HOST'];
		$this_http_or_https = (((isset($_SERVER['HTTPS']) && ($_SERVER['HTTPS'] == "on"))
			or (isset($_SERVER['HTTP_X_FORWARDED_PROTO'])
				and $_SERVER['HTTP_X_FORWARDED_PROTO'] === 'https')) ? 'https://' : 'http://');
	...
	public function verify_license($time_based_check = false, $license = false, $client = false)
	{
		...
		if ($time_based_check && $this->verification_period > 0) {
			...
			$today = date('d-m-Y');
			if (empty($_SESSION["64d4410f951d469"])) {
				$_SESSION["64d4410f951d469"] = '00-00-0000';
			}
			if ($type == 1) {
				$type_text = '1 day';
			} elseif ($type == 3) {
			...
			if (strtotime($today) >= strtotime($_SESSION["64d4410f951d469"])) {
				$get_data = $this->call_api(
					'POST',
